  ...
  registerAppTool(
    ...
    'render_view',
    {
      ...
      description: [
        ...
        'says it better than a sentence.',
        'Style it only with the host CSS variables, and only with these names, because they are',
        'the only ones a host sends. Colours: --color-background-NAME, --color-text-NAME and',
        '--color-border-NAME where NAME is primary, secondary, tertiary, inverse, ghost, info,',
        'danger, success, warning or disabled; and --color-ring-NAME with the same list minus',
        'tertiary, ghost and disabled. Type: --font-sans, --font-mono, and --font-weight-normal,',
        '--font-weight-medium, --font-weight-semibold, --font-weight-bold. Shape:',
        '--border-radius-xs, -sm, -md, -lg, -xl, -full, --border-width-regular, and',
        '--shadow-hairline, --shadow-sm, --shadow-md, --shadow-lg.',
        'There is no --color-success and no --color-danger. The semantic colours are',
        '--color-text-success, --color-background-danger and so on. A name that is not on this',
        'list arrives as nothing, so its fallback paints and the colour becomes yours rather',
